"""
FastAPI REST API Server for Cold-Boot Attack DRAM Remanence & Key Recovery Defense Agent.
"""
from typing import Dict, Any
from .models import FrontierPayload
from .agents import ColdBootCoordinator

coordinator = ColdBootCoordinator()
...
def create_app():
    try:
        ...
        app = FastAPI(
            title="Cold-Boot Attack DRAM Remanence & Key Recovery Defense Agent",
            description="Simulates low-temperature DRAM capacitor decay, bit-flip error correction, and AES key schedule reconstruction from decay traces.",
        ...
        @app.get("/health")
        def health():
            return {"status": "HEALTHY", "system": "cold-boot-dram-remanence-agent", "domain": "Hardware Security", "version": "2.0.0-FRONTIER"}
